...
unsigned int theory_time = 0;	// время, равное времени последнего прерывания, для диска вращающегося со скоростью 1 об./мин.
...
void setup()
{
  ...
  Serial.println("Time\t\tAngle");	// отладочный вывод
}
...
void loop()
{
    ...
    // Отладочный вывод
    Serial.println((String)inter_time + "\t" + (String)angle);
    //
    delay(1000);
}
