...
void setup()
{
  ...
  attachInterrupt(0, OnInterruptionAtFirst, RISING);      //Прерывание по нарастающему фронту на D0
  attachInterrupt(1, OnInterruptionAtSecond, RISING);     //Прерывание по нарастающему фронту на D1
}
...
void loop()
{
  if (flag == true)
  {
    ...
    tft.print(complete_difference * 10.0, 6);
...
// Выполняется при прерывании на DO
void OnInterruptionAtFirst()
{
  time1_prev = time1;
  time1 = micros();
  complete_difference++;
...
// Выполняется при прерывании на D1
void OnInterruptionAtSecond()
{
  time2 = micros();
  complete_difference--;
  flag = true;
