...
double start_angle = 0;         // Начальное значение угла
...
void setup()
{
  ...
  attachInterrupt(0, OnFirstStart, RISING);
  attachInterrupt(1, OnSecondStart, RISING);
}

void loop()
{
    unsigned long m = times1[i1] > times2[i2] ? times2[i2] : times1[i2];
    current = complete_difference * 10 + start_angle + interpAngle(m, times1 , times2, i1, i2);
...
void setStartAngle()
{
  start_angle = interpAngle(0, times1, times2, 2, 2);

  // Отладочный вывод
  Serial.println("StartAngle: " + (String)start_angle + " is " + (start_angle > 10 ? "not" : "") + " correct");
  //
